...
def error(text, data=None, status=400):
    data = data or {}
    data["success"] = False
    data["error"] = text
    return HttpResponse(
...
def succeed(data=None, status=200):
    data = data or {}
    data["success"] = True
    return HttpResponse(
        json.dumps(data),
    ...
    def __call__(self, func):
        def new_func(request, *args):
            if request.method in self.accepted_methods:
                ...
                # Catches other weird cases
                except ValueError as err:
                    return error("ValueError", data={"internalError": str(err)})
                # JSON errors
